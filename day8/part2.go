...
func main() {
	...
	for i, _ := range lines {
		accumulator = 0
		copy := append([]line{}, lines...)
		switchOp(&copy[i].operation)
		if err := run(0, &copy[0], copy); err == nil {
			break
		}
	}
...
func run(index int, l *line, lines []line) error {
	...
	return run(newIndex, &lines[newIndex], lines)
}
